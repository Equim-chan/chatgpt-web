...
<template>
  <template v-if="isUser">
    <NAvatar v-if="isString(avatar) && avatar.length > 0" round :src="avatar" :fallback-src="defaultAvatar" />
    <NAvatar v-else round :src="defaultAvatar" />
  </template>
  <template v-else>
    <NAvatar v-if="isString(aiAvatar) && aiAvatar.length > 0" round :src="aiAvatar" />
